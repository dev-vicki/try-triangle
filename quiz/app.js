...
quizForm.addEventListener("submit", (e) => {
  ...
  for (let element of data) {
    ...
    if (element[1] == correctAns[index]) {
      questionBlocks[index].style.background =
        "linear-gradient(to bottom right, #4eb84c, #27ae60)";
      score++;
    } else {
      questionBlocks[index].style.background =
        "linear-gradient(to bottom right, #e74c3c, #ff2b2b)";
    }
    index++;
  }
...
resetBtn.addEventListener("click", () => {
  ...
  document.getElementById("result").innerText =
    "Note : Your Score will be displayed here";
});
